...
function popularKPICards(kpis) { if (!kpiCardsContainer) return; Array.from(kpiCardsContainer.querySelectorAll('.card:not(.facturacion-card)')).forEach(card => card.remove()); const kpisRelevantes = ['VOZ', 'FO', 'N° TERMINALES', 'ACCESORIOS', 'SEGUROS']; const hoy = new Date(), diaAct = hoy.getDate(), diasMes = new Date(hoy.getFullYear(), hoy.getMonth() + 1, 0).getDate(); let kpiCardElements = []; kpisRelevantes.forEach(nomKpiRel => { const kpiD = kpis.find(k => k.kpi && k.kpi.trim().toUpperCase() === nomKpiRel.trim().toUpperCase()); const iconSvg = KPI_ICONS[nomKpiRel.toUpperCase()] || KPI_ICONS['DEFAULT_KPI']; const kpiIdentifier = nomKpiRel.toUpperCase().replace(/[^A-Z0-9\s]/g, '').replace(/\s+/g, '_'); let cardHTML = `<div class="card">`; cardHTML += `<div class="kpi-icon-wrapper" aria-hidden="true">${iconSvg}</div>`; if (kpiD) { const meta = parseFloat(kpiD.valor100) || 0, resConArr = parseFloat(kpiD.total) || 0; const esMon = nomKpiRel.toUpperCase().includes('ACCESORIOS'); let porcCumpl = meta > 0 ? (resConArr / meta) * 100 : (resConArr > 0 ? 100 : 0); porcCumpl = Math.max(0, porcCumpl); const propDiasTr = diasMes > 0 ? diaAct / diasMes : 0, metaEspHoy = meta * propDiasTr, resActSinArr = parseFloat(kpiD.resultado) || 0; let colorB = (resActSinArr >= meta && meta > 0) || (meta === 0 && resConArr > 0) ? 'verde' : (resActSinArr >= metaEspHoy ? 'amarillo' : 'rojo'); let proyeccionValor = 0; if (diaAct > 0 && resConArr > 0 && diasMes > 0) proyeccionValor = (resConArr / diaAct) * diasMes; else if (resConArr > 0) proyeccionValor = resConArr; const proyeccionHTML = `<p class="detalle-kpi proyeccion-kpi">Proyección: ${esMon ? '$' : ''}${formatNumber(proyeccionValor)}</p>`; const cardActionsHTML = `<div class="card-actions"><button class="btn-details" data-kpi="${kpiIdentifier}">Ver más</button></div>`; cardHTML += `<h3>${kpiD.kpi}</h3><p class="valor">${esMon ? '$' : ''}${formatNumber(resConArr)}</p><div class="barra-progreso-kpi"><div class="progreso-kpi ${colorB}" style="width:${Math.min(porcCumpl,100)}%;" title="${porcCumpl.toFixed(1)}%">${porcCumpl > 15 ? porcCumpl.toFixed(0) + '%' : ''}</div></div><p class="detalle-kpi">Meta: ${esMon ? '$' : ''}${formatNumber(meta)} (${porcCumpl.toFixed(1)}%)</p>${proyeccionHTML}${cardActionsHTML}`; } else { cardHTML += `<h3>${nomKpiRel}</h3><p class="valor">N/D</p>`; } cardHTML += `</div>`; const tempDiv = document.createElement('div'); tempDiv.innerHTML = cardHTML.trim(); if (tempDiv.firstChild) { kpiCardElements.push(tempDiv.firstChild); } }); const factCard = kpiCardsContainer.querySelector('.facturacion-card'); kpiCardElements.forEach(cardEl => { if (factCard) { kpiCardsContainer.insertBefore(cardEl, factCard); } else { kpiCardsContainer.appendChild(cardEl); } }); kpiCardElements.forEach((card, index) => { requestAnimationFrame(() => { setTimeout(() => { card.classList.add('card-visible'); }, index * 75); }); }); }
